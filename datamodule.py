...
class BaseDataModule(pl.LightningDataModule):
    ...
    def rand_obs(self, gt_item):  # to generate random observations ONLINE
        ...
        for t_ in range(dtime):
            ...
            if np.sum(obs_mask_item_t_) > 0.25 * dlat * dlon:
                obs_obj = 0.25 * np.sum(obs_mask_item_t_)     ## 0.25 is the treshold for the ratio of missing data (covering more than 75%)
    ...
    def setup(self, stage="test"): ## In the OSSE setup, we apply the mask on all the datasets
        ...
        self.test_ds = XrDataset(
            ...
            **self.xrds_kw,
                postpro_fn=post_fn if (stage == 'test') else post_fn_rand
        )
...
def artificial_clouds(dataset, mask, pct):
    time_ = dataset.time.data
    dlat = len(dataset.lat.data)
    dlon = len(dataset.lon.data)

    new_ds = []

    for i in range(len(time_)):
        day = time_[i]
        img = dataset.sel(time = day).SPM.data
        obs_mask_item = ~np.isnan(img)
        pixels_mask   = np.sum(~np.isnan(mask))
        _obs_item = img
        
        if np.sum(obs_mask_item) > pct * (dlat * dlon - pixels_mask):
                obs_obj = pct * np.sum(obs_mask_item)     ## 0.25 is the treshold for the ratio of missing data (covering more than 75%)
                while np.sum(obs_mask_item) >= obs_obj:
                    half_patch_height = np.random.randint(2, 10)
                    half_patch_width = np.random.randint(2, 10)
                    idx_lat = np.random.randint(0, dlat)
                    idx_lon = np.random.randint(0, dlon)
        ...
        obs_mask_item = obs_mask_item == 1
        obs_item = np.where(obs_mask_item, _obs_item, np.nan)
        new_ds.append(obs_item)
    
    new_ds = np.array(new_ds)

    new_dataset = xr.Dataset(
       { 
           'SPM': (['time', 'lat', 'lon'], new_ds)
        },
        coords = {
        'time': time_,
